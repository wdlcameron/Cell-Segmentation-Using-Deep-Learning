...
class ImagingParameters():
    ...
    class ImageClass():
        ...
        def __init__(self, directory, filename, channel_array, image_type_array, channel_thresholds):
            ...
            shape = self.reader.get_data(0).shape
            #if len(shape) == 3: _ , self.height, self.width = shape
            #else: self.height, self.width = shape
    ...
    def output_cache(self, cache, file_dir, filename):
        ...
        premask_images = cache['Premask Images']   #Shape [ROI][h:w:ch]

        data_folder = self.data_path
        debug_folder = data_folder/'Debug' ; debug_folder.mkdir(exist_ok = True, parents = True)
        save_folder = debug_folder

        sub_folder = file_dir.relative_to(self.data_path)
        save_folder = debug_folder/sub_folder/filename.parent
        name_base = filename.name.replace(f".{self.suffix}", '')


        print('Outputting the cache:', file_dir, filename)
        ...
        for ROI_num in range(len(sub_images)):
            #Export the cell image and masks
            output_folder = save_folder/'Masks' ; output_folder.mkdir(exist_ok = True, parents = True)
 
            #Export the cell image
            cell_exp = premask_images[ROI_num]
            cell_exp = (cell_exp/np.amax(cell_exp)*255).astype('uint8')
            imageio.imwrite(output_folder/f'{name_base}_{ROI_num}.jpg', cell_exp)

            #Export the mask
            mask_exp = cell_masks[ROI_num]
            imageio.imwrite(output_folder/f'{name_base}_{ROI_num}.png', mask_exp.astype('uint8')*255)
            ...
            for colour_num in range(len(sub_images[ROI_num])):
                ...
                imageio.imwrite(output_folder/f'{name_base}_{ROI_num}.ome.tif', stacked_img)

                #export the jpegs
                jpeg_export = (export_img/np.amax(export_img)*255).astype('uint8')
                jpeg_export = np.dstack([jpeg_export,jpeg_export,jpeg_export])
                imageio.imwrite(output_folder/f'{name_base}_{ROI_num}.jpg', jpeg_export)


                #export the masks (x255 to see them and edit them easily)
                sub_export = sub_masks[ROI_num][colour_num][CH]
                imageio.imwrite(output_folder/f'{name_base}_{ROI_num}.png', sub_export.astype('uint8')*255)
    ...
    def iterate_through_files(self, directory_list, dataframes = {}):
        ...
        for root_dir in sorted(directory_list):
            ...
            for files in sorted(directory_list[root_dir]):                  
                ...
                if debug_mode: self.output_cache(debug_cache, root_dir, files)
