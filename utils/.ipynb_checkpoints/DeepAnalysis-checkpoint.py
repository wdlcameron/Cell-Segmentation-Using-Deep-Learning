...
def deep_analysis(current_Image, Parameters, dataframes):
    ...
    for time in range(timepoints):
        ...
        #0) Open entire image
        raw_image = read_image(reader, time, range(images_per_timepoint)) #Shape (H, W, Channels)
        ...
        else: ROI_list = detect_ROIs(Parameters, detection_raw_image)
        

        
        
        if debug_mode: debug_seg, debug_array = create_debug_images(raw_image, ROI_detection_channels, channel_array)
        ...
        for ROI in ROI_list:
            ...
            if debug_mode == True: debug_seg = annotate_image(debug_seg, ROI, np.zeros(ROI.data.shape))
            ...
            ROI.data = cell_mask
            if debug_mode == True: debug_seg = annotate_image(debug_seg, ROI, ROI.data)
        ...
        if debug_mode == True: export_debug_images(Parameters, current_Image, debug_seg, debug_array)
